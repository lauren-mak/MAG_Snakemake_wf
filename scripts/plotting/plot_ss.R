...
subplots_quast <- function(df) {
    p = ggplot(df, aes(x = value, y = variable, colour = Assembly)) + 
...
graph_quast <- function() {
    ...
    plot_1a = subplots_quast(quast_mod[quast_mod$variable == "Total_length",]) + 
        scale_y_discrete(labels=c("Total_length" = "Total\nAssembly\nLen. (bp)"))

    # Plot 1b: Number of (binnable) contigs
    plot_1b = subplots_quast(quast_mod[quast_mod$variable == "Num_contigs",]) +
        scale_y_discrete(labels=c("Num_contigs" = "Number of\nContigs"))

    # Plot 1c,d: Proportion of binnable vs. unused contigs and lengths
    plot_1c = subplots_quast(quast_mod[quast_mod$variable == "Avg_len_used",]) + 
        scale_y_discrete(labels=c("Avg_len_used" = "Proportion of\nBinnable\nAssembly"))
    plot_1d = subplots_quast(quast_mod[quast_mod$variable == "Avg_ctg_used",]) + 
    ...
    plot_2a = subplots_quast(quast_mod[quast_mod$variable == "Largest_contig",]) +
        scale_y_discrete(labels=c("Largest_contig" = "Longest\nBinnable\nContig (bp)"))
    plot_2b = subplots_quast(quast_mod[quast_mod$variable == "Avg_binned_contig",]) +
        scale_y_discrete(labels=c("Avg_binned_contig" = "Average\nBinnable\nContig (bp)"))
    plot_2c = subplots_quast(quast_mod[quast_mod$variable == "Avg_all_contig",]) +
        scale_y_discrete(labels=c("Avg_all_contig" = "Average of All\nContigs (bp)")) +
        theme(legend.position="bottom", legend.title=element_text(size=rel(1.2)), legend.text=element_text(size=rel(1.0)))

    # Plot 3: NX and LX
    plot_3a = subplots_quast(quast_mod[quast_mod$variable == "N50" | quast_mod$variable == "N75",])
    plot_3b = subplots_quast(quast_mod[quast_mod$variable == "L50" | quast_mod$variable == "L75",]) +
        theme(legend.position="bottom", legend.title=element_text(size=rel(1.2)), legend.text=element_text(size=rel(1.0)))
